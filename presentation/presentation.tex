  ...
  \begin{minipage}[c]{.5\linewidth}
    ...
    \begin{assumptions}
      \begin{itemize}[<+(1)->]
        ...
        \item Attack is monotonically increasing \pause
              $\lambda_{b}>\lambda_{a}\to\gamma(\lambda_{b})>\gamma(\lambda_{a})$
      \end{itemize}
    \end{assumptions}
    \begin{itemize}[<+->]
      \item Invertible
      \item If $\lambdaicheat=\Tik\lambdai$ $\to \exists \Tik^{-1}$
    \end{itemize}
  \end{minipage}
\end{frame}
...
\begin{frame}{What are deprived systems?}{}
  ...
  \begin{minipage}[t]{.35\linewidth}
    ...
    \onslide<3->{
      \begin{equation*}
        \begin{matrix}
          \alert<4-5>{\minimize\limits_{\vec{U}_{i}[k]}}&\alert<4-5>{\frac{1}{2}\norm{\vec{U}_{i}[k]}^{2}_{H_{i}} + {\vec{f}_{i}[k]}^{T}\vec{U}_{i}[k]}\\
          \mathrm{subject~ to} & \bar{\Gamma}_{i}\vec{U}_{i}[k] \only<-6>{\preceq}\only<7->{\alert<7>{=}} \thetaik:\lambdaik
        \end{matrix}
      \end{equation*}
    }
  \end{minipage}
