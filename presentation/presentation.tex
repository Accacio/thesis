...
\begin{frame}{Context}{Requirements evolve with time}
  ...
  \only<1-4>{
    ...
    \begin{tikzpicture}
      ...
      \node (image) at (0,0) {\includegraphics<4->[width=0.5\textwidth]{city_streets_layer.pdf}};
      \node (image) at (0,0) {\includegraphics<2->[width=0.5\textwidth]{city_houses_layer.pdf}};
      \node (image) at (0,0) {\includegraphics<3->[width=0.5\textwidth]{city_buildings_layer.pdf}};
    ...
    \begin{tikzpicture}
      \node (image) at (0,0) {\includegraphics<1->[width=\textwidth]{city_grass_layer.pdf}};
      \node (image) at (0,0) {\includegraphics<3->[width=\textwidth]{city_streets_layer.pdf}};
      \node (image) at (0,0) {\includegraphics<2->[width=\textwidth]{city_houses_layer.pdf}};
      \node (image) at (0,0) {\includegraphics<4->[width=\textwidth]{city_buildings_layer.pdf}};
      \node (image) at (0,0) {\includegraphics<1->[width=\textwidth]{city_trees_layer.pdf}};
      \node (image) at (0,0) {\includegraphics<6->[width=\textwidth]{city_energy_layer.pdf}};
      \node (image) at (0,0) {\includegraphics<7->[width=\textwidth]{city_heat_water_layer.pdf}};
      \node (image) at (0,0) {\includegraphics<8->[width=\textwidth]{city_cars_layer.pdf}};
    \end{tikzpicture}
  \end{minipage}
  \hfill
  \only<6-9>{
  \begin{minipage}{.5\textwidth}
    \begin{itemize}
      \item<6-> Electricity Distribution System
      \item<7-> Heat distribution
      \item<7-> Water distribution\\
      \item<8-> Traffic management
      \item[]<9->(include your problem here)
    \end{itemize}
  \end{minipage}
  }
  \only<10->{
  \begin{minipage}{.5\textwidth}
    \begin{itemize}
      \item<10-> Multiple systems interacting
      \item<11-> Coupled by constraints
      \item<12-> Optimization objectives
            \begin{itemize}
              \item<13-> Minimize energy consumption
              \item<14-> Maximize user satisfaction
              \item<15-> Follow a trajectory
              \item[]<16-> \dots
            \end{itemize}
      \item<17-> Solution $\to$ MPC
    \end{itemize}
  \end{minipage}
  }
...
\begin{frame}{Model Predictive Control}{In a nutshell}
  ...
  \script<5>{following what we call the receding horizon strategy. However this problem is not always straightforward to solve, for some cases it can be easier.
}
    ...
    \only<8-10>{
      \begin{minipage}[c]{.4\linewidth}
        ...
        \begin{matrix}
          ...
          \mathrm{subject~ to} & \bar{\Gamma}_{1}\vec{U}_{1}[k] \preceq \tikzmark{theta1}\mpcvec{\theta}[1][k]:\mpcvec{\lambda}[1][k]
          \\
          &\vdots&
          \\
          \minimize\limits_{\vec{U}_{M}[k]}&\frac{1}{2}\norm{\vec{U}_{M}[k]}^{2}_{H_{M}} + {\vec{f}_{M}[k]}^{T}\vec{U}_{M}[k]\\
          \mathrm{subject~ to} & \bar{\Gamma}_{M}\vec{U}_{M}[k] \preceq \tikzmark{thetam}\mpcvec{\theta}[M][k]:\mpcvec{\lambda}[M][k]
        \end{matrix}
        \end{equation*}
      \end{minipage}
      \hfill
...
\newcommand{\speechthis}[2]{
  \tikz[remember picture,baseline]{
    \node[anchor=base,inner sep=0,outer sep=0] (oi) {};
      }%
    }%
  ...
  \only<2->{\begin{minipage}{.45\linewidth}
    \begin{figure}
      ...
      \begin{tikzpicture}[balloon/.style={
        ...
        \visible<6>{\node[balloon,fill=blue!50, callout relative pointer={(0,1)}] at ($(image.center)+(-.5cm,0.8cm)$) {Guys,\\ let's compromise ...};}
      \end{tikzpicture}
    \end{figure}
  \end{minipage}}
...
\begin{frame}{Mitigating}
  \begin{overlayarea}{\textwidth}{.5\textwidth}
    There are vulnerabilities\\
    \only<4->{``Great''. But what can we do?}
      ...
      \begin{minipage}[t]{.45\linewidth}
        \only<6->{
          \begin{figure}[h]
            ...
            \caption*{\alert<7>{Recover original behavior}.}\label{fig:minimum_recovered}
          \end{figure}
        }
      \end{minipage}
    }
...
\begin{frame}{Deprived Systems}{But why?}
  \centering
  \begin{itemize}
    \item<2-> No Scarcity \onslide<3->{$\to$ All constraints satisfied} \onslide<4->{ $\to$ $\lambda_{i}=0$} \onslide<5->{$\to$ No coordination needed}
    \item<6-> Scarcity $\to$ Competition \onslide<7->{$\to$ Consensus/Compromise} \onslide<8->{(or cheating {\color{red}\faUserSecret})}
  \end{itemize}
\end{frame}
...
\begin{frame}{Estimating $\Plintildeestimate$}
  \centering
  \begin{itemize}
...
\begin{frame}{Mitigation mechanism}
  \begin{itemize}
...
\begin{frame}{Example}
  \begin{exampleblock}{District Heating of 4 Distinct Houses Under Power Scarcity}
    \begin{itemize}
            \pause
...
\begin{frame}{Artificial Scarcity}
  \centering
