...
\begin{frame}{Estimating \small $\Plintildeestimate$}
  ...
  \begin{itemize}[<+(1)->]
    \item Challenge: Online estimation during negotiation fails
          ...
          \end{itemize}
    \item Solution: Send a random\footnote{A random signal has persistent excitation of any order (\visible<.(1)->{\booksymbol~\citetitle{AstroemWittenmark1989}})} sequence to increase excitation until convergence.
  \end{itemize}
  ...
  \onslide<+(1)->{
    \begin{equation*}
      ...
      \begin{aligned}
        \lambdaik=
        \begin{cases}
          {-\Plinineq\thetaik-\sikineq},&\text{if}\ \thetaik \in\alert<.(3)>{\set{R}_{\lambdai}^{0}}\tikzmark{a}\\
          \qquad\quad \vdots&\qquad\quad \vdots\\
          -\Plinineq[i][2^{\nineq}-1]\thetaik-\sikineq[i][2^{\nineq}-1],&\text{if}\ \thetaik \in\alert<.(3)>{\set{R}_{\lambdai}^{2^{\nineq}-1}}\\
        \end{cases}
      \end{aligned}
    \end{equation*}
  }
...
\begin{frame}{Artificial Scarcity}{Who is it? Who is it?}
  ...
  \begin{assumption}
    \onslide<+(1)->{
      We known a point $\thetairestricted$ which activates all constraints\footnote{If we have local constraints, we suppose this point respects them. }
    }
  \end{assumption}
  \onslide<+(1)->{
...
\begin{frame}{Results}{Temporal}
 ...
 \begin{figure}[h]
    \centering
    \includegraphics[width=1.\textwidth,trim=0 .3cm 0 .2cm,clip]{../img/resilient_ineq/ErrorWX_command_normErrH.pdf}
    \caption*{Temperature in house I. \\Error  $E_{I}(k)$.\\ {\encircle{N}} Nominal, {\encircle{S}} Selflish {\encircle{C}} Corrected}
  \end{figure}
  \end{minipage}
  \only<2->{
    \begin{minipage}[c]{.50\linewidth}
      \begin{figure}[h]
        \centering
        \reflectbox{\includegraphics[width=.5\textwidth,trim=0 .3cm 0 .2cm,clip]{../img/mutley.jpg}}
      \end{figure}
    \end{minipage}
  }
...
\begin{frame}{Conclusion}{Main takeaways}
  \begin{itemize}
    \item How can an agent attack? \onslide<3->{{\color{mpc_green} \faCheck}}
  \begin{itemize}
    \item<2-> Attacker can change the communication to receive more ressources.
  \end{itemize}
    \item What are the consequences of an attack? \onslide<5->{{\color{mpc_green} \faCheck}}
  \begin{itemize}
    \item<4-> Suboptimality and maybe instability
  \end{itemize}
    \item Can we mitigate the effects? \onslide<7->{{\color{mpc_green} \faCheck}}
  \begin{itemize}
...
\begin{frame}{Open question Future Directions}
    \begin{itemize}[<+(1)->]
      \item Partial/incremental reconstruction of cheating matrix
      \item Study of robustness + noise
