  ...
  \begin{table}[H]
    \resizebox{.9\textwidth}{!}{
      \begin{tabular}[h]{l!{\vrule}cc<{\onslide<4->}c<{\onslide<4->}c<{\onslide}}
        ...
        \midrule
        \only<5->{\\[-\normalbaselineskip]\rowcolor{ietrRed!10}}\LARGE Our &\LARGE \alert<6->{Primal} & \LARGE \alert<7->{Resilient} & \LARGE Active Analyt./Learn. & \LARGE \alert<8->{Data reconstruction}\\
        \bottomrule
      \end{tabular}
    }
  \end{table}
...
\begin{frame}{Outline}
  ...
  \begin{itemize}[<+(1)->]
    ...
    \item \encircle{3} yielded \cite{NogueiraEtAl2022} (NecSys'22)
  \end{itemize}
...
\begin{frame}{Artificial Scarcity}{Who is it? Who is it?}
  ...
  \begin{assumption}
    \onslide<+(1)->{
      We know a point $\thetairestricted$ which activates all constraints\onslide<+(1)->{\footnote{If we have local constraints, we suppose this point respects them.}}
    }
  \end{assumption}
  ...
  \onslide<+(1)->{
    ...
    \only<+(1)->{
      \begin{minipage}[c]{.55\linewidth}
        ...
        \end{itemize}
        \begin{itemize}
          \item<+(1)-> How to known the radius?
                \begin{itemize}
                  \item<+(1)-> We don't.
                \end{itemize}
