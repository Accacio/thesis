...
\begin{frame}{How can a non-cooperative agent attack?}{Our approach}
  \begin{overlayarea}{\textwidth}{.2\textwidth}
    \begin{minipage}[t]{.6\linewidth}
      ...
      \end{itemize}
      \centering
      \only<5->{$\tilde{\vec{\lambda}}_{i}=\gamma_{i}(\vec{\lambda}_{i})$\\}
      \only<6>{Let's suppose $\gamma_{i}(\vec{\lambda}_{i})=T_{i}$}
    \end{minipage}
    \hfill
  ...
  \begin{minipage}[c]{.45\linewidth}
    \begin{figure}[!t]
      ...
      \begin{tikzpicture}
        ...
        \draw<5>[rounded corners,fg,thick] (-2.3,2.5) rectangle +(.5,-5);
        \draw<6>[rounded corners,fg,thick] (2.3,2.5) rectangle +(.75,-5);
      \end{tikzpicture}
    \end{figure}
  \end{minipage}
  \hfill
...
\begin{frame}{Strategy}
  \begin{itemize}
    \item Recover original behavior
          \begin{itemize}
            \item Invert effects of function $\gamma_{i}(\vec{\lambda}_{i})$ \onslide<2>{$\to$ Estimate $T^{-1}$}
          \end{itemize}
...
\begin{frame}{Deprived Systems}{But why?}
  \centering
  \begin{itemize}
    \item<2-> No Scarcity \onslide<3->{$\to$ All satisfied} \onslide<4->{ $\to$ $\lambda_{i}=0$} \onslide<5->{$\to$ No coordination needed}
...
\begin{frame}{Deprived Systems}{Analysis}
   \begin{itemize}
    \item<2-> We can transform inequality constraints into equality ones\footnote{Under some conditions, \todo{see here} }
