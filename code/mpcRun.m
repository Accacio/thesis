...
plot(0,xt(1,1:K),'^','LineWidth',1,'Color',([1 1 1]+[0.47 0.12 0.22])/2)
...
matlab2tikz(['../img/mpcState' num2str(1) '.tex'], 'height', '\fheight', 'width', '\fwidth','showInfo', false );
close

figure
stairs(-1:0,[0,0],'--','LineWidth',1,'Color',([1 1 1]+[0.47 0.12 0.22])/2)
hold on
stairs(-1:0,[0,0],'k-','Color',[0.47 0.12 0.22])
...
matlab2tikz(['../img/mpcControl' num2str(1) '.tex'], 'height', '\fheight', 'width', '\fwidth','showInfo', false);
close
...
%   subplot(2,1,1);
    ...
    patch([K K K+n K+n],[18 22 22 18],[.47 .12 .22],'FaceAlpha',.1,'EdgeColor','none')
    hold off
    ylabel('Temperature (^oC)')
    xlim([1 simK])
    ylim([18 22])
    ...
    set(hl,'FontSize',18)
    title('Temperature in Room')
    matlab2tikz(['../img/mpcState' num2str(K+1) '.tex'], 'height', '\fheight', 'width', '\fwidth','showInfo', false );
...
%     subplot(2,1,2)
    ...
    matlab2tikz(['../img/mpcControl' num2str(K+1) '.tex'], 'height', '\fheight', 'width', '\fwidth','showInfo', false);
