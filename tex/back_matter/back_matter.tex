...
Dans ce travail, nous nous intéressons aux attaques \fdi{}, où avec un peu de connaissance sur le système, l'attaqueur modifie le trafique d'informations pour altérer son comportement pour son propre bénéfice.
...
Pour les méthodes d'apprentissage, on peut citer~\cite{AnandutaEtAl2018,AnandutaEtAl2019,AnandutaEtAl2020} qui utilises une approche bayésiènne pour apprender quelques bornes et détecter agents suspects.
...
\begin{remark}
Comme indiqué en~\cite{ArauzEtAl2021} il est important d'observer que le choix du borne de détection influencie les faux positifs associés.
\end{remark}
...
Ces stratégies font le système se comporterr pas forcement de la manière originale, mais d'une manière sub-optimale satisfaisante, jusque la normalité soit restaureée.
...
En~\cite{VelardeEtAl2017a,VelardeEtAl2017b,VelardeEtAl2018}, les auteurs présentent des méthodes robustes baséees sur la théorie des graphes, où l'attaqueur n'est pas detecté par soi, mais agents suspects sont déconnectés ou ignorés.
...
La stratégie est plus détailée dans les prochaines sections, mais pour la détection, on utilise une méthode \textbf{active}, où nous ajoutons des données pour détecter l'attaqueur..
...
\begin{table}[H]
  ...
  \begin{tabular}[h]{lccccc}
    ...
    \midrule
    \cite{VelardeEtAl2017a} & Duale & Oui & Robuste (Scénario) & NA & NA\\
...
Le problème~\eqref{eq:DOP_main_resumé} est résolu par mises à jour de $\theta_{i}$ jusqu'à une convergence.
...
On suppose qu'avant chaque négociation, l'attaqueur décide une fonction ${\gamma:\R^{c}\times\R\to \R^{c}}$, pour modifier $\lambdai$ avant de l'envoyer au coordinateur.
...
On simule pour différent scénarios.
Pour chaqe scénario agent $3$ utilise un coefficient de triche $\tau_{3}[k]$ distinct.
...
Quand $\tau_{3}$ augmente, les fonctions objectif de l'attaqueur ($\Jiacc[3]$) décroît au même moment que celles des autres augmentent.
L'opposé se passe pour des valeurs de $\tau_{3}$ entre $0$ et $1$.
Dans ce cas l'attaqueur détériore sa propre performance, ce qui ne justifie pas tel attaque, sauf dans un scénario de détournement.
