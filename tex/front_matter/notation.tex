...
\begin{description}
  ...
  \item[$\elemend$] The last element of row or column of the entity.
  ...
  \item[$\dagger$] Pseude-inverse such ${A^{\dagger}={{(A^{T}A)}^{-1}A^{T}}}$
  \item[$\vectorize{A}$] Vectorizes matrix $A\in\R^{m\times n}$, such $\vectorize{A}=[\elem[1,*]{A},\dots,\elem[n,1]{A}]\T$
  \item[$\abs{\cdot}$ $\norm{\cdot}_{n}$ $\norm{\cdot}_{F}$] Absolute value, and $\ell_n$ ($\ell_{2}$ if index suppresed) and Frobenius norms
...
\begin{description}
  ...
  \item[$\vec{v}^{(p)}$ ] Vector $\vec{v}$ at the $p$-th iteration in an algorithm
\end{description}
