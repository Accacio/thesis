...
To this end, we begin with an analysis of the system under attack.
...
\begin{remark}
  ...
  So, to find the equivalent $\bar{\Gamma}_{\text{eq}}$ (or $\bar{\Gamma}_{i_{\text{eq}}}$) for the equality constraint problem, only the active halfspaces are kept and transformed into the intersection of hyperplanes (equality constraint).
...
That means the solution to the constrained problem is the same as the unconstrained, so the problem is, in fact, unconstrained.
...
The problem is solved as shown in \S\ref{sec:uncoupled_problems}, i.e., each sub-system solves~\eqref{eq:qp_standard_form_unconstrained_solution} and the solution is already given.
...
When the system suffers from scarcity, the agents are forced to compete and compromise, which may result in attacks as shown in \S\ref{sec:primal_decomposition} and their dire consequences.
...
To find this solution, we usually use the Lagrange duality, so $\lambdaik$ will also have an analytical solution in this case.
...
As we can see, we can separate the function into two parts, one which depends on $\thetai$ and the other which does not:
\begin{equation}
...
If we analyze the columns, on the other hand, we see that they do not sum to $1$.
This means if the system converges, it will not necessarily converge in such a way the $\theta_{i}$ are equal.
...
Once $\rho\p$ is well tuned for the corresponding system, the matrix $\mathcal{A}_{\theta}$ always evolves in the same way.
So if there is a change in this evolution, it means the system presents an anomalous behavior.

As can be observed, any disturbance in $\Plin$ (change in the objective function or constraints) changes the values of $\mathcal{A}_{\theta}$, thus the system's dynamic.
This change may destabilize it if it drives the eigenvalues outside the unit circle, as shown in \S\ref{sec:attack-interest}.
...
If we suppose the system is under attack, we again use the attack model~\eqref{eq:linear_attack_reprise}.
...
Unfortunately, this method gives us only detection, but not necessarily from which agent the anomaly comes, which could be useful for a mitigation algorithm.
...
We can interpret it as if the cheating matrix $\Tik$ alters both $\Plin$ and $\sik$.

Since $\Plin$ is constant, any change between two different negotiations is a consequence of anomalous behavior, in this case, caused by $\Tik$.
...
\begin{remark}
  ...
  For instance, it needs to be bigger than the estimation error in the attack-free scenario.
...
For instance, we can choose any method in the \LS{} family~\cite{AstroemWittenmark1989}.
...
However, since the values of $\Plin$ should not vary from one negotiation to other, if we use the last estimation as the initial condition for a \RLS{} algorithm, we can gain in the time for estimation convergence.
\begin{remark}
  Observe that since we are simultaneously estimating $\sik$, which changes w.r.t. time $k$ for smaller systems, the difference between convergence times may be negligible.
\end{remark}
...
Since the exact parameter values are not known initially, an initial estimate is used.
...
Initially, we use~\eqref{eq:lambda_function_theta_tilde} directly as a regression model, transforming it into the usual form
...
Using this method, we need to estimate all elements of $\Plintildeestimate$ and $\siktildeestimate$, which results in ${c^{2}+c}$ elements.
But as the $\Plintildeestimate$ are symmetric, we can estimate only their upper triangle, reducing the number of total estimated elements to ${\frac{c^{2}+3c}{2}}$.
...
Observe that as much we could be tempted to use the algorithm to estimate during the negotiation, such an algorithm would not work throughout the negotiation exchanges.
The fact that $\lambdai$ depends on $\thetai$ and vice-versa makes them both behave as \dt{} system (as we saw in~\S\ref{sec:analysis-negotiation}).
...
As seen in~\cite{AstroemWittenmark1989}, this makes the solution of the \LS{} problem not unique, and the regressor matrix becomes singular.
...
\begin{remark}
  Other considerations about the initialization of $\rlsparam$ and the choice of forgetting parameter $\rlsforget$ can be made but is out of the scope of this work.
\end{remark}
...
In the next section, we consider some hints on how to mitigate the effects while changing the decomposition minimally, and after the analysis, we propose a mitigation method.
...
As we saw, from the coordinator's perspective, the attack comes from the modification of $\lambdai$ by the attacking agent.
So, the first idea is that instead of using the $\lambdai$ received from the attacker, the coordinator uses a modified version, here denoted $\lambdaimodified$.
...
The projection yields the constrained solution to the problem (blue dot).
\begin{figure}[h]
  \centering
...
Once one of the agents attacks the system using~\eqref{eq:linear_attack_reprise}, the level curves are distorted, as seen in Fig.~\ref{fig:minimum_after_attack}.
...
If the attacker is detected, one of our options to mitigate this drift is to substitute the $\lambdaicheat$ received during the negotiation by $\0$.
...
The negligence against the attacker will result in it receiving fewer and fewer resources.
Asymptotically, the system will behave as if the attacker was unplugged since the negotiation will be maintained with the other agents, somehow similar to the examples in~\cite{VelardeEtAl2018,MaestreEtAl2021}.
...
Although the idea of punishing the attacker by ignoring it may appear reasonable, since the system is a \cps{}, it may control some amenity that needs a minimal \QoS{}.
...
However, it would be necessary to assess somehow what would be a fair amount to give to the attacker, which would not prejudice other agents.

We propose yet another solution, in which we try to recover the nominal behavior if some assumptions are guaranteed.
Passing from the level curves of the attacked system (Fig.~\ref{fig:minimum_after_attack}) to the shown in Fig.~\ref{fig:minimum_recovered}, where the new minimum lies in a neighborhood of the original one (in Fig.~\ref{fig:original_minimum}).
...
\begin{remark}
  Observe that if the estimates of $\Plin$ do not converge, we cannot reconstruct $\lambdaik$, so we need to use one of the other options proposed, ignore the attacker, or use minimal allocation.
  Since it may be difficult to assess the minimal allocation, we ignore the cases when the estimation does not converge.
\end{remark}
...
Since we do not suppose there is any cooling apparatus, the inputs are restricted to be positive, i.e., $\vec{u}_{i}[k]\succeq\0$. Meaning that if we need to cool the environment, we can only use the thermic properties of the building.
...
Current sources are added to correspond to solar irradiation and heating power.
A voltage source represents the temperature outside the house.
The meaning of each component is given in Tab.~\ref{tab:modelParamMeaning}, and the respective values for each one of the houses are given in Tab.~\ref{tab:modelParam}.
...
In scenario S, we can observe the tracking error ${w_{I}-y_{I}}$ reducing when the attack is activated, suggesting that the attacker drives the negotiation to get more resources.
On the other hand, the other agents receive fewer resources, as we can see the tracking error increase in Fig.~\ref{fig:response_housesII_to_IV_3Scenarios}.
...
We can also compare the accumulated local and global objectives $\Jiacc$ and $\Jacc$ for the scenarios N, C, and S in Tab.~\ref{tab:eq_costsGlobalLocal}.
As expected, when no secure \dmpc{} is applied, and the system is attacked, the objective function of the attackers decreases, while the others increase, together with the global objective $\Jacc$.
...
The analysis gave us some insight into how to detect the attacks and mitigate given consequences of the attacks.
