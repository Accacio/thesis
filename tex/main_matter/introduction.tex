...
Devices with the same (or greater) computation power that took man to the moon are within a hand's reach today. Many are used to control machines everywhere.
...
For other more common methods, such \mpclong{} (\mpcshort)~\cite{GarciaEtAl1989}, this development meant solving more significant problems in less time. Some of those problems are solved even in real-time~\cite{BesselmannEtAl2008}, using small computation units smaller than coins~\cite{BanguraMahony2014}.
...
However, as we will see, only a few studies have been made on the security of the \dmpc{} strategies when units do not communicate honestly.
