...
\begin{algorithm2e}[h]
  ...
  \quad$\vec{\lambda}_{i}^{\star}(\vec{\theta}\p)$, if $\mathfrak{d}_{i}=0$ and ${\vec{\lambda}_{i}}_{\mathrm{rec}}$, if ${\mathfrak{d}_{i}=1}$~\eqref{eq:lambda_reconstruction}\;
  $p\gets p+1$
 }
...
The attack models used for scenarios~\ref{it:case_selfish} \todo{and~\ref{it:case_selfish_recovered},
are}
\begin{align}
  T_{I}=&\begin{cases}
          \todo[verify]{\left[\input{../data/triche_matrix_I}\right]},&\text{if }k> \input{../data/selfish_time_I.tex}\\
          I_{c},&\text{otherwise}
        \end{cases}\\
  T_{IV}=&\begin{cases}
          \todo[verify]{\left[\input{../data/triche_matrix_IV}\right]},&\text{if }k> \input{../data/selfish_time_IV.tex}\\
          I_{c},&\text{otherwise}
        \end{cases}.
\end{align}
...
Notice how the reference $w_{I}$ is not reached even in the nominal behavior (in \todo{magenta}),
...
\begin{figure}[h]
  \centering
 \includegraphics[width=.7\textwidth,trim=0 .3cm 0 .2cm,clip]{airtemp_roomI/__ErrorWX_command_normErrH.pdf}
  \caption{Air temperature in house I and the decision variable $E_{I}[k]$ for different scenarios.\todo{remake figure}}\label{fig:response3Scenarios}
\end{figure}
...
If the proposed resilient algorithm is activated (\todo{black dots}), the temperatures approach their nominal value $y_{I}^{N}$.
We can also compare the inputs in Fig.~\ref{fig:control_3Scenarios}.
When agent~I is selfish, its control increases while other rooms' control decrease,
...
\begin{figure}[h]
  \centering
 \includegraphics[width=.7\textwidth,trim=0 .1cm 0 .3cm,clip]{airtemp_roomI/control.pdf}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=.7\textwidth]{AirTempAndDecisionVariable.pdf}
...
\begin{table}[h]
  \centering
  \caption{Objective functions $J_{i}$ (\% error).\todo{\note{remove \% from nominal}}}\label{tab:costsGlobalLocal}
  \begin{tabular}[t]{cccc}
    \toprule
    Agent  & Scenario N& Scenario S & Scenario C\\
    \midrule
    \input{../data/table_costs_all_rooms_error.tex}\\
