...
\begin{remark}
  Usually, we make the distinction between the dual variables associated with the inequality and equality constraints by using different symbols.
...
Again, since there is only one type of lagrangian multiplier, we will call it simply the dual variable.
...
So the complete value of $\lambdaik$ will depend on the permutation of the status of the inequality constraint.
...
By removing a set of constraints, we can solve a similar problem but with a reduced number of elements in $\thetai$ and $\lambdai$.
...
To make all equations the same-sized, we recover the elements of $\lambdai$ set to zero and set aside.
...
To facilitate the notation, let us use a binary representation such that if we have $\nineq$ constraints, we use $\nineq$ binary digits to represent their status.
...
We can then use base 10 to represent such partitions.
...
\end{remark}
\newpage
\subsubsection{Considerations about status of constraints}\label{sec:cons-about-stat}
It is important to observe that there are some sets of constraints in which the permutation is not necessarily $2^{\nineq}$.

For example, in some cases, there is no partition where all constraints are inactive, i.e., the intersection of halfspaces is empty (see Fig.~\ref{fig:non_feasible}).
...
In other cases, there is no combination where all constraints are active.
We give some different examples.
For instance, for constraints with normals $\vec{\vec{\eta}}_{i}$, if the angles of adjacent halfspaces $\vecangle{\vec{\eta}_{i}}{\vec{\eta}_{j}}$ is $180^{o}$ and the intersection is not nil (see Fig.~\ref{fig:parallel_only_one_active}), at least one of the constraints will always be inactive.
\begin{figure}[H]
  \centering
  \begin{tikzpicture}
    \def\ang{25}
...
$\set{S}$ being the intersection of halfplanes, does not have a straightforward solution as in the equality constraint case~\eqref{eq:example_projectedSubgradient_lambda_reprise}.
The solution is found by applying the definition of the Euclidean projection, which is by itself a \qp{} problem
...
\begin{remark}
  Probably some of those permutations may not be possible, but still, we have exponential growth in the number of modes of the switched systems.
...
This consecutive choice of assumptions reduces the total number of switching modes to exactly one, presented by equations~\eqref{eq:negotiation_equation_substituting_organizing_matrix_lambda} and~\eqref{eq:negotiation_equation_substituting_organizing_matrix_theta}, reproduced here.
...
And thus, the solutions in ~\eqref{eq:negotiation_equation_substituting_organizing_matrix_lambda} and~\eqref{eq:negotiation_equation_substituting_organizing_matrix_theta} can be seen as a specific case of the following solutions for the $0$-th modes:
\begin{equation}
  ...
  \begin{cases}
    ...
    \mathcal{A}_{\lambda}^{N_{\vec{\lambda}}}\vec{\lambda}[k]\p,&\text{if}\ \vec{\theta}\in\set{R}_{\vec{\lambda}}^{N_{\vec{\lambda}}}\\
  \end{cases}
\end{equation}
\begin{equation}
...
The $0$-th case, as mentioned, is the same as we found in the last chapter, giving
...
\begin{remark}\label{rem:cheat_satisfied_problem}
  One interesting fact we can observe is that for cases where the corresponding $\lambdai$ is already $\0$, the multiplication of $T_{i}$ will not affect its value.
  A satisfied agent, i.e., an agent whose local problem has the same solution as its unconstrained version, has no motive to get more resources since the allocated resources will satisfy its needs.
...
If we have $M=2$ subsystems, one in the $1$-zone and the other in the $2$-zone (let us call them agents I and II), and we suppose the solution of the projection is in its $0$-zone, i.e., ${G_{\vec{\mu}}^{0}[k](\vec{\theta}[k]\p+\rho\p\vec{\lambda}[k]\p) \preceq \vec{U}_{\max}^{0}[k]}$, we code the permutation as
\begin{equation}
...
\begin{remark}
  Observe that since the matrices $\mathcal{A}_{\vec{\theta}}^{n}$ and $\mathcal{A}_{\vec{\lambda}}^{n}$ may be sparse, the eigenvalue analysis to indicate the stability of the mode is accomplished by keeping only the non-zero corresponding blocks.
\end{remark}
...
Here are proposals on how to counter the difficulties.
...
However, estimating $\Tikinvestimate$ is a bit more difficult than before.
...
If we estimate any $\Plinineq[i][n]$, we cannot necessarily invert it.
...
So, we assume that $\Plinineq$ exists and that it is possible to estimate it.

The necessary condition for the existence of the $0$-th region ($0$-zone) is that we can choose $\thetai$ such that the constraints $\inequalityfunction$ are active for the unconstrained solution $\optuncUik$.
...
And supposing there is a value $\thetairestricted$ that activates all constraints, we call it the \emph{artificial scarcity point}.
...
Therefore, we need an estimation method that can also simultaneously classify the points, indicating to which zone they correspond.
...
The problem is formalized in an optimization problem which we adapt for our case.
...
\end{equation}
where $g:\R^{\card{\pwaestsysinput}}\to\{1:\pwaestnumparam\}$ is a linear classification function, which classifies the membership of its argument, returning the corresponding index of the region it is a member of.
The definition of such a function can be seen in~\cite{LauerBloch2019}.
...
So, the algorithm solves the optimization problem in an iterative manner.

Instead of estimating the $\set{P}$ that maximizes ${\ln\probability{\set{B};\set{P}}}$, we find estimates that maximize the expectation of the complete-data log-likelihood $\ln\probability{\set{B},\set{U};\set{P}}$ w.r.t.
...
calculate it using current estimates of $\set{P}$ denoted $\set{P}_{\mathrm{cur}}$.
And repeating the same strategy with the newfound estimate, we can approach the estimates which maximize the log marginal likelihood.

The successive steps of calculating the posterior probabilities (expectation) and maximizing the complete-data log-likelihood give the method the name Expectation Maximization.
...
The iterative algorithm can be summarized in Algorithm~\ref{alg:em}.
...
The idea is to incorporate probabilistic behavior into our model~\eqref{eq:lambdafuntheta}.
...
We can then construct our regression model as
...
In~\cite{FariaSoromenho2010}, a somehow similar model is called \emph{mixture of linear regressions}, but since we are using \pwa{} functions, which have linear and constant terms, we will call the model \emph{mixture of affine regressions}.

Since we don't know the partitions or the belonging of each observed point, we associate the index $z$ of each observation ${(\randomvec{\lambda}_{o}, \randomvec{\theta}_{o})}$ to a latent random variable ${\random{z}_{o}}$.
...
\begin{remark}
  We estimate $\pi^{z}$ and $\tilde{\vec{s}}^{z}$ purely for updating the model and constraining the identification of $\tilde{P}^{z}$, since they are used neither for the detection nor for the mitigation phases.
\end{remark}
...
An example of the representation of such function for a $1$ dimension \pwa{} function can be seen in Fig.~\ref{fig:affine_gaussian_mixture}, where darker points present more probability than lighter ones.
\begin{figure}[h]
  \centering
...
To facilitate the math, we introduce a variable $\vec{\phi}^{z}$ to embed the parameters, similar to what we made for the \RLS{} case (\S\ref{sec:about-estimation})
\begin{equation}
  \vec{\phi}^{z}=\left[
      \begin{matrix}
...
Other methods, such as \sEM{} and \CEM{}, make a hard choice based on the responsibilities.
...
\begin{remark}
  Observe that the parameters indices $z$ do not necessarily converge to the parameters with the same number in~\eqref{eq:lambdafuntheta} since it depends on the initialization of $\set{P}$ and the observation set.
  For example, if we generate points inside one single region $j$, all $Z$ regions would have the estimated parameter of the given region $j$.
\end{remark}
...
The other is the dependability of the solution to the initial estimate conditions, which was already commented on in Remark~\ref{rem:em_depends_on_initialization}.
...
The order depends on the number of observations $O$ and a number of zones $Z$, and some variables can come close to being singular.

One way to counteract this effect is to include it in the estimated parameters.
Another way is to consider the covariance as a diagonal matrix with the same values for all entries~\cite{KarlisXekalaki2003}.
...
\begin{algorithm2e}[h]
  ...
  \detectPhase{
    ...
    \For{$o\in\set{O}$}{
    Coordinator sends $\vec{\theta}_{i}^{o}\in\set{B}(\thetairestricted,r)$\;
    Subsystems solve~\eqref{eq:DOP_local}, and send $\tilde{\vec{\lambda}}^{o}_{i}$\;
    }
    Coordinator estimates $\Plinineqtildeestimate$ and $\sikineqtildeestimate$ with \EM{} (Algorithm~\ref{alg:em})\;
    Coordinator computes $\mathfrak{d}_{i}$ using~\eqref{eq:detection_ineq}\;
  }
  \negotPhase{
    Coordinator initializes $\vec{\theta}^{(p)}$ and sends to subsystems\;
  ...
  Coordinator updates allocation~\eqref{eq:projectedSubgradient_lambda_reprise} using $\lambdaimodified$:\;
  \quad$\vec{\lambda}_{i}^{\star}(\vec{\theta}\p)$, if $\mathfrak{d}_{i}=0$ and $\lambdaireconstructed$, if ${\mathfrak{d}_{i}=1}$~\eqref{eq:lambda_reconstruction}\;
  $p\gets p+1$
 }
...
As a study case, we use the same example of \dhn{} presented in \S\ref{sec:case-study}, with slight modifications to respect the assumptions made in this chapter.
...
As noted in \S\ref{sec:case-study}, we do not have cooling apparatus.
Thus the inputs are restricted to be positive, i.e., $\vec{u}_{i}[k]\succeq\0$, so
...
\begin{equation}
  \label{eq:houses_artificial_deprived_local_problem}
  \begin{matrix}
...
We test the algorithm using the same simulation period of
...
\begin{align}
  T_{I}=&\begin{cases}
          \left[\input{../data/resilient_ineq/triche_matrix_I}\right],&\text{if }k> \input{../data/resilient_ineq/selfish_time_I.tex}\\
          I_{c},&\text{otherwise}
        \end{cases}
  % T_{IV}=&\begin{cases}
...
In Fig.~\ref{fig:response3Scenarios_all_houses}, we see how the resources from house~I were transferred from all other houses, which lost performance.
...
displaying a shift in the negotiation, and giving more resources to the attacker.
...
% \todo{
%   We can also show the method works similarly for cases MS and MC, where multiple agents attack the system (Fig.~\ref{fig:response_houses_N_M_as}),
% }
% \begin{figure}[h]
%   \centering
%   \includegraphics[width=.7\textwidth]{../img/resilient_ineq/__ErrorWX_command_normErrH.pdf}
% \caption{Temperature in all houses, and respective references for scenarios N, MS and MC. \todo{remake figure}\note{only if i have time, probably not}}\label{fig:response_houses_N_M_as}
% \end{figure}


Another way to represent the performances of our proposition, however, more palpably is to compare the accumulated objective functions during the period of simulation for each of the three scenarios (Table~\ref{tab:costsGlobalLocal}).
...
Our affine regression model becomes a \pwa{} regression model, and we need to use an adequate estimation method that can estimate parameters using such a regression model.

We presented the \em{} algorithm,  which, although it may be adequate for such estimation, uses a probabilistic model. We adapt our model to respect its needs and use it to estimate the parameters needed for our detection and mitigation phases.

Finally, we presented our resilient algorithm using artificial scarcity and tested it using our \dhn{} study case.

% \todo{In the next chapter we discuss how to relax the artificial scarcity assumption.\note{if I have time, probably not}}
