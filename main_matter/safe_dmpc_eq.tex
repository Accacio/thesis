...
\begin{equation}
  \rlssysinput=\left[
    \begin{matrix}
      \text{\structDataSym}(\thetai\p) &  I_{\card{\thetai\p}}
    \end{matrix}
  \right].
\end{equation}
...
Using the construction above and applying to the \RLS{} algorithm in~\cite{AstroemWittenmark1989}, we can describe an iteration of the \RLS{} algorithm to estimate the symmetric matrix $\Plintildeestimate$ and the vector $\siktildeestimate$ simultaneously.
Algorithm~\ref{alg:update_rls} shows how to calculate a new parameter estimate $\rlsparam$.

\begin{algorithm2e}[h]
  \DontPrintSemicolon%
  \KwIn{Current estimation step $p$, Forgetting rate $\rlsforget$, Current parameter estimate $\rlsparam\p$, Current gain $\rlsgain\p$, Current input $\thetai\p$, Current output $\rlssysoutput\p$}
  \KwOut{New estimate $\rlsparam\p$}
  $\rlssysinput\gets\left[\text{\structDataSym}(\thetai\p),I_{\card{\thetai\p}}\right]$\;
  Calculate residual $\rlsresidual$: $\rlsresidual\gets \rlssysoutput-\rlssysinput\rlsparam$\;
  Update gain $\rlsgain$: $\rlsgain\pplusone\gets \rlsforget^{-1}\rlsgain\p-\rlsforget^{-2}\rlsgain\p\rlssysinput\T{(I_{\card{\thetai\p}}+\rlsforget^{-1}\rlssysinput\rlsgain\p\rlssysinput\T)}^{-1}\rlssysinput\rlsgain\p$\;
  Calculate new estimate $\rlsparam$ $\rlsparam\pplusone\gets\rlsparam\p+\rlsgain\pplusone\rlssysinput\T\rlsresidual$\;
...
Using all the knowledge harvested, we can systematize the detection technique in a algorithm.

\begin{algorithm2e}[h]
  \DontPrintSemicolon%
  \KwIn{Nominal $\Plinnominal$, Maximum number of iterations $h_{\max}$, Bound $\epsilon_{\rlsparam}$, Bound $\epsilon_{\Plin}$, }
  \KwOut{Detection $d_{i}$}
  $h\gets0$\;
  \Repeat{$\left[\norm{\rlsparam\pplusone-\rlsparam\p}\leq\epsilon_{\rlsparam}\right] ||\ \left[h\geq h_{\max}\right]$}
  {
    Coordinator sends random $\vec{\theta}_{i}^{(h+1)}$ to $i$ \;
    Subsystem solves~\eqref{eq:DOP_local}, and sends $\vec{\lambda}^{\star}_{i}(\vec{\theta}^{(h)})$ back\;
    Coordinator updates estimates $\rlsparam$ using Algorithm~\ref{alg:update_rls}\;
    $h\gets h+1$\;
  }
  Reconstruct $\Plintildeestimate$ from $\rlsparam$\;
  $E_{i}[k]\gets\norm{\Plintildeestimate-\Plinnominal}_{F}$\;
...
\end{algorithm2e}
\todo{maybe use negotiation + random noise}.
as on the makes the evolution of the system
