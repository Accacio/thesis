...
We can interpret as the cheating matrix $\Tik$ alters both $\Plin$ and $\sik$.

Since $\Plin$ is constant, any change between two different negotiation is a consequence of an anomalous behavior, in this case caused by $\Tik$.
...
\section{Considerations about parameter estimation}\label{sec:about-estimation}
As commented, we can suppervise the exchanges and gather multiple values of $\thetai$ and $\lambdai$ to serve as an input dataset to the chosen estimation method.
