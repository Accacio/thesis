...
\begin{equation}
  \label{eq:negotiation_equation_substituting_organizing}
  \vec{\theta}\pplusone=\mathcal{A}\vec{\theta}\p+\mathcal{B}[k]
\end{equation}
where
\begin{equation}
\mathcal{A}=\left[
...
\right]
\end{equation}
which is a \dt{} system with forces input ($\mathcal{B}$).
...
If we suppose the system is attacked, any attack in $P_{i}$ (objective function or constraint) changes the system's dynamic (change in $\mathcal{A}$), and may destabilize it if drives the eigenvalues outside the unit circle, as shown in \S\ref{sec:attack-interest}.
On the other hand attacks in $\vec{s}_{i}[k]$ (objective function, constraint or state/reference) may change the steady state value (change in $\mathcal{B}$).
...
\begin{equation}
  ...
  \vec{\theta}\pplusone=\tilde{\mathcal{A}}[k]\vec{\theta}\p+\tilde{\mathcal{B}}[k]
\end{equation}
where
\begin{equation}
...
Unfortunately, this method gives us only a detection, but not necessarily from which agent the attack comes.
