...
\begin{example}[Effects of a malicious agent]\label{ex:qualitative_example}
  ...
  For this a number of \emph{agents interact}.
  ...
  \end{figure}
Observe in Fig.~\ref{fig:ex_exchange_agents} that agent 1 sends a suspicious $\lambda_{1}$. Instead of sending the message it was supposed to send, it modifies it first and then it sends this modified version. This can be considered as an attack \todo{called \fdi, as we will see later}.
...
From this simple example we can see the two main effects of a attack: Loss of optimality and eventually \todo{total breakdown} of the system.
...
\\Then, in Chapter~\ref{sec:safe_pddmpc_ineq}, we analyze a similar problem with just one small modification, which as we will see increases exponentially the complexity of the problem. From the analysis we propose similar mechanisms, but with the
