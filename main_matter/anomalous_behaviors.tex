...
Passive strategies are usually based on robust control, where a control is implemented which can maintain satisfactory performance if values of some variables are inside a given safety range (robust against disturbances) or if a number of components present anomalous behaviors.
...
For example in distribution grids there is N-1 static security index, which indicates how stable the grid is even if 1 component present a failure (or is disconnected)~\cite{QianEtAl2022}.
...
While usually for specific kinds of attacks new resilient methods need to be created in order to mitigate the effects.
