...
To prevent attacks and faults, we secure the vulnerabilities in each component.
\\Here we can give some examples.
Physical tampering can be prevented by surveilling the physical components, enclosing them by walls and doors with access control whenever possible \todo[]{Citation~\cite{DingEtAl2018}??}.
For faults due to deterioration, it is usual to do periodically preventive maintenance \todo{citation??}.
