...
\begin{figure}
  ...
  \begin{tikzpicture}[
    ...
    \draw[-latex,thick] (controller.west) -- ++(-1,0) node [midway,above] {u} |- (plant.west) ;
...
\begin{figure}
  ...
  \begin{tikzpicture}[
    ...
    \node[draw,cloud,aspect=2,cloud puffs=20,below=1cm of controller] (network)  {Network};

    \draw[-latex,thick] (controller.west) -- ++(-1,0) node [midway,above] {u} |- (plant.west) ;

    \draw[-latex,thick] (plant.east) -- ++(1,0) node [midway,above] {y} |- (controller.east);

    \draw[-latex,thick] (controller.south -| network.puff 2) -- (network.puff 2) node [midway,left] {$\mathcal{I}_{\text{out}}$};

    \draw[latex-,thick] (controller.south -| network.puff 20) -- (network.puff 20) node [midway,right] {$\mathcal{I}_{\text{in}}$};
...
\begin{figure}
  ...
  \begin{tikzpicture}[circuit ee IEC,
    ...
    \draw[thick] (controller.west) -- ++(-1,0) node (a) {} node [midway,above] {u};
    \draw[thick] (a.center) to[make contact] (a.center |- plant.west) node (b) {};
    \draw[-latex,thick] (b.center) -- (plant.west);

    \draw[thick] (plant.east) -- ++(1,0) node (aa) {} node [midway,above] {y};
    \draw[thick] (aa.center) to[make contact] (aa.center |- controller.east) node (bb) {} ;
    \draw[-latex,thick] (bb.center) -- (controller.east);

    \draw[thick] (network.puff 2) node (nout) {}  to [make contact](controller.south -| network.puff 2) node (nout2) {};
    \node[above left=-4pt and -2.5pt] at (nout) {$\mathcal{I}_{\text{out}}$};

    \draw[thick] (controller.south -| network.puff 20) node (nin) {} to[make contact] (network.puff 20) node (nin2) {};
    \node[above right=-5pt and 0pt] at (nin2) {$\mathcal{I}_{\text{in}}$};

\end{tikzpicture}
\caption{Attack in networked \cps{}.}\label{fig:attacks_networked_cps}
\end{figure}
\todo{um boneco para cada lugar com numeros diferentes e explicar}

In~\cite{TeixeiraEtAl2015} the authors emphasize the control and physical aspects of \cps{} (sensors, actuators, model dynamics etc) and how this knowledge can be useful for the attacker. Also they emphasize how the attacks not necessarily happen purely.
So, to categorize aspects they proposed an attack space based on 3 dimensions: Model Knowledge, Disruption resources and disclosure resources as seen in Fig.~\ref{fig:3_dimensions_attack}.
\usetikzlibrary{3d}
