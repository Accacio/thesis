...
These kinds of attacks are called \textbf{inter-agent attacks}.
And usually the objectives are accomplished by deception.
...
While both terms are usually used interchangeably here we make a distinction.

\textbf{Robustness} can be defined as the ability to withstand perturbations without change in function~\cite{Jen2003}, while \textbf{resilience} is the ability to maintain an accepted level of operational normalcy  in  response  to  disturbances,  including  threats  of  an unexpected and malicious nature\cite{Rieger2010}.
...
\todo{remove}
Some techniques can be applied to create attack/fault-tolerant systems or to robustify the systems against them\cite{DingEtAl2018,SatchidanandanKumar2017,BoemEtAl2020}

Fault tolerant control\cite{VahidNaghaviEtAl2014}
...
We usually protect components with more probability of presenting an anomalous behavior~\cite{Bishop2005} (risk and threat analysis).

\paragraph{Prevention} To prevent anomalous behaviors, we secure components that we see as a possible threat due its vulnerabilities.
...
~\\\\When prevention fails it is time to use a strategy, based on resilience (active), robustness (passive) or even both

\paragraph{Detection}
Use some system knowledge to recognize an anomalous behavior.

\todo{Detection based on residuals of state dynamics~\cite{TeixeiraEtAl2015}}
In \cite{PasqualettiEtAl2013}, the authors discuss what are monitors and how attacks can be detected and identified.
\todo{or detectors, Other kind of detectors can be found in~\cite{ZhangEtAl2021a}}
...
Fault Diagnoses~\cite{BoemEtAl2020}
...
In~\cite{VelardeEtAl2018} their solution is based on f-robustness of graphs\cite{DibajiIshii2015,WangIshii2019} and ignoring extreme values. Similarly~\cite{AnandutaEtAl2018,AnandutaEtAl2019,AnandutaEtAl2020} base their solution on f-local graph robustness, their method detects suspect agents using a bayesian approach and ignore them in the negotiation.
The approach in~\cite{VelardeEtAl2017a,MaestreEtAl2021} is to robustify the system using probability of occurence of trustworthy historical scenarios.
