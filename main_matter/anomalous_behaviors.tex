...
In a control context~\cite{CardenasEtAl2008}, the authors used only the first 3, calling them Deception, Disclosure, and~\DoS{} instead of disruption.
In~\cite{AminEtAl2009}, the authors used only \textbf{deception} and \textbf{denial of service} to divided attacks, probably because disclosure attacks do not affect performance.
In~\cite{TeixeiraEtAl2015} the authors emphasize the control and physical aspects of \cps{} (sensors, actuators, model dynamics etc) and how this knowledge can be useful for the attacker.
This model of categorization is usually called the \DDD{} model.

A general abstraction used for \cps{} to demonstrate the attacks and their category is to divide them into communication channel, controller and the Physical system (as seen in Fig.~\ref{fig:cps_abstraction}).
...
In Fig.~\ref{fig:networked_cps_abstraction} we represent all other systems as a single entity called network with which the system can communicate.
We also add two other signals, which represent the information shared with the network, $I_{\text{in}}$, information from all other systems and $I_{\text{out}}$ information given to all other systems.
...
Using this general networked control system scheme, one way to characterize the attacks is by where the attack happen.
In Fig.~\ref{fig:attacks_networked_cps} we can see multiple kinds of attacks enumerated from A1 to A9.
The attacks from A1 to A4 are deception attacks, where the attacker changes the signals to a modified version (here represented by a \~{} above the variable name).
The attacks from A5 to A8 are disruption attacks, where the attacker momentarily or not prevents the communication between two entities.
\todo{It can be divided also into denial of service and denial of ...}
And A9 are physical attacks, where the attacker change physically the behavior of the system.
...
In~\cite{TeixeiraEtAl2015}, the authors also stress that the attacks not necessarily happen purely.
So, instead of using the \DDD{} model, they proposed an attack space based on 3 dimensions: Model Knowledge, Disruption resources and disclosure resources as seen in Fig.~\ref{fig:3_dimensions_attack}.


\todo{um boneco para cada lugar com numeros diferentes e explicar}

\usetikzlibrary{3d}
\usetikzlibrary{perspective}
\begin{figure}
  \centering
  \begin{tikzpicture}[3d view={115}{20},
  ...
  \node[above right=5pt and -10pt] at (disclosure) {Disclosure Resources};
	...
	\draw[axis] (0,0,0) -- (0,0,5) node[above]{Model Knowledge};
  ...
  \node[above right=5pt and -10pt] at (eavesdropping) {Eavesdropping};
	...
	\node[fill,circle,inner sep=1.5pt,label=above left:DoS] at (.5,0,0) {};

	\node[fill,circle,inner sep=1.5pt] (covert) at (4,4,4) {};
  \node[above right=5pt and -30pt] at (covert) {Covert};

	\draw[cube hidden] (4,4,0) -- (4,4,4);
	\draw[cube hidden] (0,4,4) -- (4,4,4);
	\draw[cube hidden] (4,0,4) -- (4,4,4);

	\draw[cube hidden] (4,4,0) -- (4,0,0);
	\draw[cube hidden] (4,4,0) -- (0,4,0);

	\draw[cube hidden] (0,4,4) -- (0,4,0);
	\draw[cube hidden] (0,4,4) -- (0,0,4);

	\draw[cube hidden] (4,0,4) -- (4,0,0);
	\draw[cube hidden] (4,0,4) -- (0,0,4);
\end{tikzpicture}
\caption{Attacks space and some attacks. (Adpated from~\cite{TeixeiraEtAl2015})}
\end{figure}
