...
\end{tikzpicture}
\caption{Attacks in networked \cps{}. Adpated from~\cite{AminEtAl2009,DibajiEtAl2019}.}\label{fig:attacks_networked_cps}
\end{figure}
...
To illustrate, we can populate the attack space with some attacks, some already cited and others will be explained subsequently (Fig.~\ref{fig:3_dimensions_attack_with_attacks}).
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[node distance=1cm and 2cm,
    ...
    \node[thick,draw,circle,inner sep=1pt,right=1.75cm of plant_model] (y_sum)  {+};
    \node[thick,draw,circle,inner sep=1pt,left=1.75cm of covert_controller] (u_sum)  {+};
    ...
    \draw[thick] (plant_model.east) -- ++(1,0) node (model_aa) {} node [midway,above] (model_output) {$\gamma$};
    \draw[thick] (covert_controller.west) -- ++(-1,0) node (covert_a) {} node [midway,below] (covert_input) {$\mu$};

    \draw[-latex,thick] (aa.center) -- (y_sum);
    \draw[-latex,thick] (model_aa.center) -- (y_sum) node[midway,below right=.1cm] {-};
    \draw[-latex,thick] (model_aa.center) |- (covert_controller.east);

    \draw[-latex,thick] (y_sum) |- (controller.east);

    \draw[-latex,thick] (covert_a.center) |- (plant_model.west);

    \draw[-latex,thick] (covert_a.center) -- (u_sum);
