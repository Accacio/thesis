...
These kinds of attacks are called \textbf{inter-agent attacks} and sometimes \textbf{insider attacks}.
...
\section{Passive strategies}\label{sec:protecting_against_attacks}
...
As we will see, some active strategies may also take advantage of this kind of property.
...
\section{Active strategies}\label{sec:protecting_against_attacks}

For other specific kinds of anomalous behaviors, or more ingenious attacks such as replay attacks or covert attacks, some resilient methods need to be created.
...
In order to identify an anomalous behavior, first we need some prior information about the normal behavior of the system.
With this knowledge we can supervise the threatened component and create monitors.
This part is called the \textbf{Detection/Isolation} phase, where the attack can be detected and the mishaving component/agent can be isolated.

Once an attack detected, the control law changes in order to mitigate the effects of the anomalous behavior.
This is called the \textbf{Mitigation} or \textbf{Recovery} phase.

In the next subsection we try to better illustrate some examples of mechanisms for the two phases.

\subsection{Detection/Isolation}

Some techniques can be applied to create attack/fault-tolerant systems or to robustify the systems against them~\cite{BoemEtAl2020}
...
Detection of covert attacks based on observator~\cite{HoehnZhang2016}
