...
In this chapter we describe in more details the optimization decomposition framework used in this work, founded on the definitions given in chapters~\ref{sec:decomposing_mpc} to~\ref{sec:anomalous} and giving insights of interpretation of each step.
...
\begin{remark}
  To pass from the global version of the variables in~\eqref{eq:qp_standard_form_reprise} to the local version in~\ref{eq:qp_standard_form_decomposable}, we apply a given linear transformation $Z^{T}Z=I$ to $\vec{U}[k]$ to reorder its components, such
  \begin{equation}
...
\begin{figure}[h]
  ...
  \caption{Evolution of state $x_{i}[k]$ and reference $w_{i}[k]$ during control ${k\in\set{K}}$. \todo{remake figure}}\label{fig:example_state}
\end{figure}
