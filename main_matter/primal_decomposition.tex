...
\begin{algorithm2e}[h]
  \DontPrintSemicolon%
  \KwIn{Maximum number of iteration $p_{\max}$, Bound $\epsilon$}
...
\end{equation}
where
\input{../data/example_system_dynamics.tex}
The system is
...
We use a \dmpc{} with horizon ${\predhorz=\input{../data/example_predhorz.tex}}$,
initial conditions
\input{../data/example_states.tex}
and state references
\input{../data/example_references.tex}
and gains
$Q_{i}=10I_{2}$ and
$R_{i}=   I_{2}$
...
\begin{equation}
  \label{eq:1}
  \begin{array}{lll}
    \input{../data/example_system_params.tex}
  \end{array}
\end{equation}
...
\begin{figure}[h]
  ...
  \caption{Evolution of $\lambdai$ during iterative process for $k=0$ when agent $2$ is hijacked. \todo{remake figure}}\label{fig:example_liar_lambda1}
\end{figure}

\begin{figure}[h]
  \centering
  \includegraphics[width=.5\textwidth]{../img/example_liar_lambda_k_4.png}
  \caption{Evolution of $\lambdai$ during iterative process for $k=4$ when agent $2$ is hijacked. \todo{remake figure}}\label{fig:example_liar_lambda5}
\end{figure}
