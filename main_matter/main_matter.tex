...
Some decomposition methods can be used to make the solution of such problems more tractable, using different time-scales (\todo[hierarchical dmpc in ChristofidesEtAl2013, Abreu talvez??]{??}) for instance.
...
In the literature, it is not unusual to assume that we can choose the computing units in order to correspond to the sub-systems of the system (\todo[give examples of units]{??}), i.e., a computing unit computes the solution of a sub-problem to find the input of a corresponding sub-system.
...
Sometimes the hierarchical structure may be implied by the decomposition. For instance, some decompositions (\todo[exemplos de sequential dado em ChristofidesEtAl2013]{??}) need the agents to solve their problems one after the other until the values converge.
...
In other decompositions (\todo[exemplos de parallel dado em ChristofidesEtAl2013]{??}), there is no hierarchy and all agents solve their problems independently of others and then share results until convergence is reached, iteratively or not.
