...
We begin by the end, with the proposal for the mitigation because it is the more straightforward one.

We want to continue to mitigate the effects of the attack by reconstructing the $\lambdai$.
...
The necessary condition for existence of the region is that we can choose $\thetai$ such the constraints $\inequalityfunction$ are inactive for the unconstrained solution $\optuncUik$.
...
So, if we can generate points in a ball around $\thetairestricted$\note{choose a better notation, please} (See Fig.~\ref{fig:ball_around_theta_restricted}), we can use the same method of supervision of communication of the coordinator and agents used in the last chapter to estimate $\Plinineq$.

However, the complication is while choosing the size of the ball around $\thetairestricted$, we may traverse one of the frontiers of the polytope and generate points corresponding to other modes (See Fig.~\ref{fig:ball_around_theta_restricted_traversing}).
...
\begin{figure}[h]
  ...
  \begin{subfigure}{0.4\textwidth}
    ...
    \end{tikzpicture}
    \caption{Ball around $\thetairestricted$.}\label{fig:ball_around_theta_restricted}
  \end{subfigure}
  \hfill
  \begin{subfigure}{0.4\textwidth}
    \centering
    ...
    \end{tikzpicture}
    \caption{Ball around $\thetairestricted$ traversing zones.}\label{fig:ball_around_theta_restricted_traversing}
  \end{subfigure}
  \caption{Generating points close to $\thetairestricted$.}
\end{figure}

Therefore, we need an estimation method which can classify simultaneously the points, indicating to which zone they correspond and we take only the parameters corresponding to the $0$-zones, that is $\Plinineq$.
...
Since the coordinator controls $\thetai$ it can choose where to generate the points
