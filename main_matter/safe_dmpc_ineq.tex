...
\section{Impact on negotiation}\label{sec:impact-local-problem}
We can try to apply the same logic used in \S\ref{sec:analysis-negotiation}, by substituting~\eqref{eq:lambdafuntheta} into~\eqref{eq:projectedSubgradient_lambda_reprise} and vice-versa.
...
For notation sake we call $\vec{x}_{0}=\vec{\theta}[k]\p+\rho\p\vec{\lambda}[k]\p$.

The problem is solved similarly to how we made in \S\ref{sec:impact-local-problem}.
Skipping some steps we will find that the solution of the projection has the same form that~\eqref{eq:lambdafuntheta}.
...
This also results in a \pwa{} function for $\vec{\theta}[k]$ with at most $2^{c}$ different modes ($\card{\elem[*,1]{I_{c}^{M}}}=c$):
\begin{equation}
  ...
  \end{aligned},
\end{equation}
where $P_{\vec{\mu}}^{(n)}={({I_{c}^{M}}^{(n)}{{I_{c}^{M}}^{(n)}}\T)}^{-1}$, $\vec{s}_{\vec{\mu}}^{(n)}[k]=P_{\vec{\mu}}^{(n)}{I_{c}^{M}}^{(n)}\vec{x}_{0}$, ${I_{c}^{M}}^{(n)}$ is constructed by keeping/removing the constraints as coded by index $(n)$, and the halfspaces described by tuples $(G_{\vec{\mu}}^{n},U_{\max}^{n})$ represent the different regions of the partitions of the solution space of $\vec{x}_{0}$.

Observe that when all constraints are inactive, the solution is the same as in the equality case~\eqref{eq:euclidean_projection_equality_solution} ($\card{I_{c}^{M}}=c\times Mc$).
And when all constraints are active (coded $2^{c}-1$), the solution is the same as the unconstrained solution, since $\vec{\mu}=\0$ (see Remark~\ref{rem:sparse_solutions}).
...
As in \S\ref{sec:analysis-negotiation} when substituting the equations one in another gave us expressions of \dt{} systems, similarly here if we substitute the results for $\lambdaik$ in~\eqref{eq:projection_result_zones} we will have a switched \dt{} system with to determine the dynamics of $\thetaik$.
And conversely a switched \dt{} homogenous system for the dynamics of $\lambdaik$.
...
The case for scarce systems seen in the last chapter, settles the solution of the projection to the case $(0)$, where we project onto the intersection of all halfspaces.
