...
\end{equation}
with ${\set{S} = \setbuild{\vec{\theta}[k]}{I_{c}^{M}\vec{\theta}[k]\preceq \vec{U}_{\max}}}$.
\begin{remark}
...
This way, we can notice that the set of constraints divides the space of $\thetaik$ into multiple regions, depending if the constraint is active or not.
...
\begin{equation}
    ...
    \begin{aligned}
      ...
      \begin{cases}
        ...
        -\Plinineq[i][\nineq-1]\thetaik-\sikineq[i][\nineq-1],&\text{if}\ G^{(\nineq-1)}[k]\thetaik \preceq b^{(\nineq-1)}[k] \\
      \end{cases}
    \end{aligned}
\end{equation}
...
As one may expect, by definition, the $\Plinineq[i][n]$ and $\sikineq[i][n]$ are potentially sparse, culminating in a $\Plinineq[i][\nineq-1]=0_{\card{\lambdai}}$ and $\sikineq[i][n]=\0_{\card{\lambdai}}$.
