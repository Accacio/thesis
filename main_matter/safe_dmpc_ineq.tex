...
So, we exchange the affine functions of each zone in~\eqref{eq:lambdafuntheta} for multi-dimensional normal functions.
The mean of the normal functions are the values of our affine functions, but instead of using our original deterministic variables $\thetai$ and $\lambdai$, we use their stochastic counterparts denote $\random{\thetai}$ and $\random{\lambdai}$.
The covariance matrices $\Sigma^{z}$ tend to $0_{\card{\thetai}}$, approaching the original values of the functions.
...
This way we can define our mixture of affine regressions as
...
\begin{remark}
  Observe that the indices $z$, not necessarily coincide with the ones in~\eqref{eq:lambdafuntheta}.
\end{remark}
