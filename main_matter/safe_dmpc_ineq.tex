...
\begin{remark}
  Observe that the parameters indices $z$ do not necessarily converge to the parameters with the same number in~\eqref{eq:lambdafuntheta}, it depends on the initialization of $\set{P}$.
\end{remark}
...
The other is the dependability of the initial conditions, commented in Remark~\ref{rem:em_depends_on_initialization}.

Some other problems may be encountered, for example for the inversion of some variables (notably the covariance $\Sigma$), when working with high orders (depending on the number of observations $O$ and number of zones $Z$).
