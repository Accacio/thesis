...
\begin{equation}
  ...
  \begin{cases}
    \mathcal{A}_{\lambda}^{0}\vec{\lambda}[k]\p,&\text{if}\ G_{\vec{\lambda}}^{0}[k]\vec{\theta}[k] \preceq \vec{b}_{\vec{\lambda}}^{0}[k] \\
    \vdots\\
    \mathcal{A}_{\lambda}^{N_{\vec{\lambda}}}\vec{\lambda}[k]\p,&\text{if}\ G_{\vec{\lambda}}^{N_{\vec{\lambda}}}[k]\vec{\theta}[k] \preceq \vec{b}_{\vec{\lambda}}^{N_{\vec{\lambda}}}[k] \\
  \end{cases}
\end{equation}
\begin{equation}
  ...
  \vec{\theta}[k]\pplusone=
  \begin{cases}
    \mathcal{A}_{\theta}^{0}\vec{\theta}[k]\p+\mathcal{\vec{B}}_{\theta}^{0}[k],&\text{if}\ G_{\vec{\theta}}^{0}[k]\vec{\theta}[k] \preceq \vec{b}_{\vec{\theta}}^{0}[k] \\
    \vdots\\
    \mathcal{A}_{\theta}^{N_{\vec{\theta}}}\vec{\theta}[k]\p+\mathcal{\vec{B}}_{\theta}^{N_{\vec{\theta}}}[k],&\text{if}\ G_{\vec{\theta}}^{N_{\vec{\theta}}}[k]\vec{\theta}[k] \preceq \vec{b}_{\vec{\theta}}^{N_{\vec{\theta}}}[k] \\
  \end{cases}
\end{equation}
...
\begin{remark}
  ...
  even if all agents cheat at the same time
...
But due to the extension of the analysis (exponential number of cases) we skip it for brevity's sake, and will use an example of a case different from the trivial solution to illustrate.

If we retake our $2$-constraints example in Fig.~\ref{fig:constraints_partition_theta},
the resulting values of $\lambdaicheat$ are
\begin{equation}
  \lambdaicheat=
  \begin{cases}
    -\widetilde{\Plinineq      }\thetaik-\widetilde{\sikineq      },&\text{if in zone 0}\\
    -\widetilde{\Plinineq[i][1]}\thetaik-\widetilde{\sikineq[i][1]},&\text{if in zone 1}\\
    -\widetilde{\Plinineq[i][2]}\thetaik-\widetilde{\sikineq[i][2]},&\text{if in zone 2}\\
    -\widetilde{\Plinineq[i][3]}\thetaik-\widetilde{\sikineq[i][3]},&\text{if in zone 3}\\
  \end{cases}\label{eq:1}
\end{equation}
where
\begin{align}
  \widetilde{\Plinineq      }&=T_{i}{{(\linearcoefiineqnonzero)}^{-1}}&&=&&T_{i}{(\linearcoefi)}^{-1}\\
  \widetilde{\Plinineq[i][1]}&=T_{i} \left[
                               \begin{matrix}
                               ...
                               \end{matrix}
                                    \right]&&=&& \left[
                                             ...
                                             \end{matrix}
                                                  \right]\\
  \widetilde{\Plinineq[i][2]}&=T_{i} \left[
                               \begin{matrix}
                                 0&0\\
...
\end{align}
\begin{align}
  \widetilde{\sikineq[i][0]}&=&&T_{i}\sikineq[i][0]&&=&&T_{i}{(\linearcoefi)}^{-1}\bar{\Gamma}_{i}H_{i}^{-1}\vec{f}_{i}[k]\\
  \widetilde{\sikineq[i][1]}&=&&T_{i} \sikineq[i][1]&&=&& \left[
                                                  \begin{matrix}
                                                  ...
                                                  \end{matrix}
                                                       \right]\\
  \widetilde{\sikineq[i][2]}&=&&T_{i} \sikineq[i][2]&&=&& \left[
               \begin{matrix}
  ...
  \right]
  \\
  \widetilde{\sikineq[i][3]}&=&&T_{i}\sikineq[i][3] &&=&&\left[
                                                  \begin{matrix}
                                                    0\\ 0
                                                  \end{matrix}
                                                       \right]
