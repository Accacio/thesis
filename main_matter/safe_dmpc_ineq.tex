...
The representation of such function for a $1$ dimension \pwa{} function can be seen in Fig.~\ref{fig:affine_gaussian_mixture}, where \todo{lighter} points present more probability then \todo{darker}.
\begin{figure}[h]
  \centering
  \includegraphics[width=0.7\textwidth]{../img/pwa.pdf}
  \caption{Gaussian Mixture for 1D \pwa{} function with 2 modes.\todo{remake figure}}\label{fig:affine_gaussian_mixture}
\end{figure}
...
\begin{align}
  T_{I}=\begin{cases}
          \todo[verify]{\left[\input{../data/triche_matrix_I}\right]},&\todo{\text{if }k< 4}\\
          I_{c},&\text{otherwise}
        \end{cases}\\
  T_{IV}=\begin{cases}
          \todo[verify]{\left[\input{../data/triche_matrix_IV}\right]},&\todo{\text{if }k< 6}\\
          I_{c},&\text{otherwise}
        \end{cases}.
\end{align}
